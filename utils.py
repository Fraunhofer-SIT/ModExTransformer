...
def get_free_gpu(num_gpus=1, free_portion=0.99):
    ...
    memory_total = [int(x.split()[2]) for x in res[::2]]
    memory_free = [int(x.split()[2]) for x in res[1::2]]
    free_gpus = [i for i, j in enumerate(np.array(memory_free) / np.array(memory_total)) if j > free_portion]
    gpu_chosen = sorted(np.random.choice(free_gpus, num_gpus, replace=False))
    os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(map(str, gpu_chosen))
    return gpu_chosen
...
def infer_num_classes_from_dataset(dataset):
    for attr in ['targets', 'labels']:
        if hasattr(dataset, attr):
            return max(getattr(dataset, attr)) + 1


def infer_num_classes_from_full_model(model):
    return len(list(model.parameters())[-1])


def infer_num_classes_from_model_checkpoint(checkpoint):
